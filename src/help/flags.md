...
next, `declarations` will show all class and method lines.
...
that `less` will match any line with "o" in it, but only if had been marked with `key=mykey`.  So "show_history" is kept, but not "do_init".
